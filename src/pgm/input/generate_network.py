...
class GM_reciprocity:
    ...
    def reciprocity_planted_network(self, parameters=None):
        ...
        if self.verbose:
            print(
                f'Number of links in the upper triangular matrix: {sparse.triu(A, k=1).nnz}\n'
                f'Number of links in the lower triangular matrix: {sparse.tril(A, k=-1).nnz}'
            )
            print(
                f'Sum of weights in the upper triangular matrix: {np.round(sparse.triu(A, k=1).sum(), 2)}\n'

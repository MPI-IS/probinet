...
class CRepDyn:
    def __init__(self, undirected=False, initialization=0, ag=1.0, bg=0., rseed=0,
                 inf=10000000000.0, err_max=0.000000000001, err=0.01,
                 ...
                 out_folder='../data/output/', end_file='.dat', assortative=True, eta0=None,
                 fix_eta=False, fix_beta=False,
    ...
    def fit(self, data, T, nodes, mask=None, K=2):
        ...
        for r in range(self.N_real):
            ...
            while not convergence and it < self.max_iter:
                delta_u, delta_v, delta_w, delta_eta, delta_beta = self._update_em(data_AtAtm1,
                                                                                   data_T_vals,
                                                                                   subs_nzp,
                                                                                   denominator=None)
                it, loglik, coincide, convergence = self._check_for_convergence(data_AtAtm1,
                                                                                data_T_vals,
                                                                                subs_nzp, T, r, it,
                                                                                loglik, coincide,
                                                                                convergence,
    ...
    def _initialize(self, rng=None):
        ...
        if self.initialization == 0:
            ...
            if self.verbose:
                logging.info('u, v and w are initialized using the input '
    ...
    def _update_membership(self, subs_nz, u, v, w, m):
        ...
        if not self.assortative:
            ...
            uttkrp_DK = sp_uttkrp_assortative(self.data_M_nz, subs_nz, m, u, v, w, temporal=False)

        return uttkrp_DK

    def _check_for_convergence(self, data, data_T_vals, subs_nz, T, r, it, loglik, coincide,
                               convergence, data_T=None, mask=None):
        """
    ...
    def __Likelihood(self, data, data_T, data_T_vals, subs_nz, T, mask=None, EPS=1e-12):
        ...
        if self.T == 0:
            ...
            if mask is not None:
                ...
                if isinstance(data, skt.dtensor):
                    # to do
                    l = - (1 + self.beta0 * T) * self.lambda0_ija[sub_mask_nz].sum() - self.eta * (
                ...
                if isinstance(data, skt.dtensor):
                    # to do
                    l = - (1 + self.beta_hat[self.T] * T) * self.lambda0_ija.sum() - self.eta * (
    ...
    def output_results(self, nodes):
        ...
        np.savez_compressed(outfile + '.npz', u=self.u_f, v=self.v_f, w=self.w_f, eta=self.eta_f,
                            beta=self.beta_f, max_it=self.final_it,

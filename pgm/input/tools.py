...
def can_cast(string: Union[int, float, str]) -> bool:
    """
    Verify if one object can be converted to an integer object.
        ...
        -------
        bool : bool
           If True, the input can be converted to an integer object.
    """
...
def is_sparse(X: np.ndarray) -> bool:
    """
        ...
        then
    N > M(I + 1) #TODO: check with Martina
...
def sptensor_from_dense_array(X: np.ndarray) -> skt.sptensor:
    ...
    # Find the indices of non-zero elements in the dense tensor X.
    subs = X.nonzero()

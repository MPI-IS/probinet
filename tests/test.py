...
class Test(unittest.TestCase):
    ...
    algorithm = 'CRep'
    K = 3
    in_folder = 'src/pgm/data/input/'
    out_folder = 'src/pgm/data/output/'
    end_file = '_test'
    adj = 'syn111.dat'
    ego = 'source'
    alter = 'target'
    ...
    '''
    with open('src/pgm/input/setting_' + algorithm + '.yaml') as f:
    ...
    def test_running_algorithm(self):
        ...
        theta = np.load(self.model.out_folder + 'theta' + self.model.end_file +
                        '.npz')
        thetaGT = np.load(self.model.out_folder + 'theta_' + self.algorithm +
                          '.npz')

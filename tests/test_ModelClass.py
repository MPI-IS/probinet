...
class TestCRepHelpers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model_class.files = Path('pgm').resolve() / 'data' / 'input' / 'theta_gt111.npz'
        self.model_class.theta = np.load(self.model_class.files,
    ...
    def test_randomize_eta(self):
        self.model_class._randomize_eta(use_unit_uniform=True)  # pylint: disable=protected-access
    ...
    def test_randomize_w(self):
        ...
        if self.model_class.assortative:
            self.assertEqual(self.model_class.w.shape, (self.model_class.L, self.model_class.K))
        else:
            self.assertEqual(
